...
from oh_my_exam.pipelines.adapters.pat.splitter.cutter import SplitOptions, split_asset
from oh_my_exam.pipelines.adapters.pat.splitter.models import EXAM_BOARD, load_asset


def discover_downloaded(raw_root: Path) -> list[Path]:
    base = raw_root / EXAM_BOARD / "admissions" / "pat"
    return sorted(path for path in base.glob("????/*/pat_????_s[12]_[qm][ps].pdf") if path.is_file()) if base.exists() else []
...
def split_downloaded(
    ...
    *,
    ...
    assets = [
        ...
        if (not variants or asset.variant in variants)
