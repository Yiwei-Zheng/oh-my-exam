...
from oh_my_exam.pipelines.adapters.cie_alevel.splitter.content_extraction import ContentExtraction, extract_content_from_fitz_clips
...
def split_asset_to_images(asset: PaperAsset, raw_root: Path, output_dir: Path, options: CutterOptions) -> list[dict[str, object]]:
    ...
    try:
        ...
        raise ValueError(f"unsupported 9709 document type: {asset.document_type}")
    finally:
...
def _load_fitz():
    try:
        ...
        raise RuntimeError(
            "High-quality CIE 9709 splitting requires PyMuPDF. "
...
def _qp_content_rects(page: "fitz.Page", y0: float, y1: float) -> list["fitz.Rect"]:
    ...
    for block in _page_rawdict(page)["blocks"]:
        ...
        if _is_page_number_rect(rect, page.rect) or (rect.y0 > 700 and rect.width > 350) or (rect.width > 100 and rect.height < 3):
            continue
    ...
    for drawing in _page_drawings(page):
        ...
        if not _is_page_number_rect(rect, page.rect) and not (rect.width > 100 and rect.height < 3) and rect.intersects(allowed):
            clipped = rect & allowed
            if not (rect.y0 < allowed.y0 and clipped.height < 3):
                rects.append(clipped)
...
def _ms_is_subquestion_label_rect(page: "fitz.Page", rect: "fitz.Rect") -> bool:
    if page.rotation:
        return 58.0 <= rect.x0 <= 92.0 and 55.0 <= rect.y0 <= page.rect.height - 45.0 and rect.width <= 36.0
