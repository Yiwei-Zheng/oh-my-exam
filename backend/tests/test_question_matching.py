...
def test_builds_syllabus_tags_search_and_similar_questions(tmp_path: Path) -> None:
    ...
    assert "Kinematics" in catalog.get_question("a_level:cie:9709", 1)["topics"]
...
def test_builds_similarities_across_exam_programs(tmp_path: Path) -> None:
    database = tmp_path / "catalog.sqlite"
    topics = tmp_path / "topics.json"
    topics.write_text(json.dumps({"courses": []}), encoding="utf-8")
    with sqlite3.connect(database) as connection:
        migrate_global_catalog(connection)
        connection.executescript(
            """
            INSERT INTO exam_boards VALUES (1, 'cie', 'Cambridge');
            INSERT INTO exam_boards VALUES (2, 'uat', 'UAT-UK');
            INSERT INTO qualifications VALUES (1, 'a_level', 'A Level');
            INSERT INTO qualifications VALUES (2, 'admissions', 'Admissions');
