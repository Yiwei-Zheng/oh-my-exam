...
web\backend\.venv\Scripts\python -m pip install -e .\web\backend[test]
web\backend\.venv\Scripts\python -m uvicorn oh_my_exam_server.main:app --reload --host 127.0.0.1 --port 8000
```
...
The bilingual responsive product and question-search client lives in
`web/frontend/`. The hosted API is its sibling under `web/backend/`; the two
communicate only through versioned HTTP APIs.
It uses React/Vite, loads portable subject databases on demand, and performs
image OCR and question matching locally in the browser. Automatic marking and
paper generation remain future capabilities.
...
Create one shared isolated environment for tools before running it:

```powershell
python -m venv tools\.venv
tools\.venv\Scripts\python -m pip install -e .\tools\shared\cie -e .\tools\downloaders\alevel\cie[gui] -e .\tools\splitters\alevel\cie[gui] -e .\tools\downloaders\admissions\uat -e .\tools\splitters\admissions\uat -e .\tools\packers[gui] pytest
```

To enable OCR fallback for PDFs with damaged font encodings or raster-only
question text, install the splitter OCR extra and make sure the Tesseract OCR
executable is available on `PATH`:

```powershell
tools\.venv\Scripts\python -m pip install -e .\tools\splitters\alevel\cie[ocr]
```

Common commands from the repository root:

```powershell
tools\.venv\Scripts\python tools\splitters\alevel\cie\cie_alevel_splitters_cli.py init-db
tools\.venv\Scripts\ome-cie-alevel-downloader discover-frank-assets --start-year 2001 --end-year 2026 --workers 4 --delay 0.2
tools\.venv\Scripts\ome-cie-alevel-downloader sniff-frank-assets --subject 9709 --start-year 2002 --end-year 2025 --workers 4 --delay 0.2
tools\.venv\Scripts\ome-cie-alevel-downloader crawl --limit 100 --delay 1.5 --workers 4
tools\.venv\Scripts\ome-cie-alevel-downloader migrate-raw-layout
tools\.venv\Scripts\python tools\splitters\alevel\cie\cie_alevel_splitters_cli.py split-downloaded --subject 9709 --start-year 2024 --end-year 2024 --workers 4
tools\.venv\Scripts\python tools\splitters\alevel\cie\cie_alevel_splitters_cli.py backfill-content
...
```powershell
tools\.venv\Scripts\python tools\downloaders\alevel\cie\cie_alevel_downloader_gui.py
```
...
```powershell
tools\.venv\Scripts\python tools\splitters\alevel\cie\cie_alevel_splitters_gui.py
```
...
tools\.venv\Scripts\python -m pytest tools\downloaders\alevel\cie\tests
tools\.venv\Scripts\python -m pytest tools\splitters\alevel\cie\tests
tools\.venv\Scripts\python -m pytest tools\downloaders\admissions\uat\tests
tools\.venv\Scripts\python -m pytest tools\splitters\admissions\uat\tests
tools\.venv\Scripts\python -m pytest tools\packers\tests
```
