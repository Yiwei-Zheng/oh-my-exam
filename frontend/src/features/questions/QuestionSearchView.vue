...
async function search() {
  ...
  try {
    ...
    results.value = await apiRequest<Question[]>(
      `/api/v1/questions/search?${parameters}`,
    )
  } catch {
    results.value = []
    error.value = t('questions.loadFailed')
        ...
        >
          <div class="pane-heading">
            <h2>
              {{
                searched
                  ? t('questions.results', { count: results.length })
                  : t('questions.eyebrow')
              }}
            </h2>
          </div>
          ...
          >
            ...
            >
              ...
              >
                <span class="result-meta">{{ item.paper_key }} ·
                  {{
                    t('questions.question', {
                      number: item.question_number || item.local_key,
                    })
                  }}</span>
            ...
            <div class="question-heading">
              <div>
                ...
                <h2>
                  {{
                    t('questions.question', {
                      number: selected.question_number || selected.local_key,
                    })
                  }}
                </h2>
              </div>
              ...
              >
                ...
                >
                  <button
                    type="button"
                    @click="
                      selectQuestion({ ...item, exam_id: selectedExamId })
                    "
                  >
                    <span><strong>{{ item.paper_key }}</strong> ·
                      {{ item.question_number }}</span>
                    <span class="match">{{
                      t('questions.score', { score: matchPercent(item.score) })
                    }}</span>
...
<style scoped>
.question-workspace {
  --search-navy: var(--color-ink);
  --search-blue: var(--color-blue);
  --search-cyan: #eef1f4;
  min-height: 100dvh;
  background-color: var(--color-bg);
  background-image:
    linear-gradient(rgba(32, 55, 82, 0.035) 1px, transparent 1px),
    linear-gradient(90deg, rgba(32, 55, 82, 0.035) 1px, transparent 1px);
...
.search-stage::after {
  ...
  right: 0;
  top: -24px;
  color: rgba(17, 24, 32, 0.07);
  font: 700 clamp(64px, 9vw, 132px)/1 var(--font-utility);
  pointer-events: none;
}
.eyebrow {
  margin: 0 0 12px;
...
.result-list button.selected {
  box-shadow: 4px 0 0 var(--search-blue) inset;
}
.result-meta {
  display: block;
  margin-bottom: 8px;
  color: var(--search-blue);
  font: 600 12px/1.3 var(--font-utility);
}
.result-copy {
  display: -webkit-box;
  overflow: hidden;
...
.question-empty span {
  ...
  font: 700 clamp(48px, 8vw, 96px)/1 var(--font-utility);
}
.question-heading {
  display: flex;
  align-items: center;
...
.question-heading p {
  margin: 0 0 4px;
  color: var(--search-blue);
  font: 600 12px/1 var(--font-utility);
}
.question-heading a {
  min-height: 44px;
  display: inline-flex;
