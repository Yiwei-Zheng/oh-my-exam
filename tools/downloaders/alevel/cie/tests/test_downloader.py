...
from cie_alevel_downloader.models import PaperAsset
from cie_alevel_downloader.download import DownloadOutcome, download_asset
from cie_alevel_downloader.cambridge_catalog import CambridgeSubject, component_rules_for_subject, parse_subject_link_text, parse_subjects
from cie_alevel_downloader.cie import build_frank_cie_url, is_supported_frank_asset, load_assets
...
from cie_alevel_downloader.gui import (
...
from cie_alevel_downloader import pipeline
from cie_alevel_downloader.pipeline import filter_assets, migrate_raw_layout
...
def test_through_2025_manifest_expands_full_candidate_space() -> None:
    manifest = Path(__file__).parents[5] / "configs" / "cie_a_level_through_2025_manifest.json"
...
def test_gui_theme_setting_round_trips(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"

    save_gui_settings({"theme_mode": "dark"}, settings_path)

    assert load_gui_settings(settings_path) == {"theme_mode": "dark"}


def test_gui_theme_setting_ignores_invalid_values(tmp_path: Path) -> None:
    settings_path = tmp_path / "settings.json"
    settings_path.write_text(json.dumps({"theme_mode": "neon"}), encoding="utf-8")

    assert load_gui_settings(settings_path) == {}
