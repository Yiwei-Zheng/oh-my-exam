...
from uat_admissions_splitter.models import load_asset


def discover_downloaded(raw_root: Path) -> list[Path]:
    base = raw_root / "uat" / "admissions"
    return sorted(path for path in base.glob("*/20??/archive/*_s1_*.pdf") if path.is_file()) if base.exists() else []
...
def split_downloaded(
    ...
    *,
    ...
    assets = [asset for asset in assets if (not exams or asset.exam in exams) and (start_year is None or asset.year >= start_year) and (end_year is None or asset.year <= end_year)]
