...
from cie_alevel_splitter.layout_splitter import PageSlicePlan, SplitOptions, _fallback_crop_region, _record_split_result
from cie_alevel_splitter.content_backfill import backfill_processed_question_content
from cie_alevel_splitter.cutters.registry import get_subject_cutter
from cie_alevel_splitter.content_extraction import ContentExtraction
from cie_alevel_splitter.metadata_schema import CROP_REGION_KEYS, MS_MANIFEST_KEYS, QP_MANIFEST_KEYS, validate_manifest_schema
from cie_alevel_splitter.models import PaperAsset
from cie_alevel_splitter.paths import project_relative_path
...
def test_unified_subject_cutter_metadata_loads_in_packer(tmp_path: Path) -> None:
    ...
    packer_src = project_root / "tools" / "packers" / "src"
    if str(packer_src) not in sys.path:
        sys.path.insert(0, str(packer_src))
    from exam_packer.metadata_loader import load_metadata_records
...
def test_backfill_processed_question_content_updates_existing_sidecar(tmp_path: Path) -> None:
    ...
    pdf_path = project_root / "data" / "raw_papers" / "cie" / "a_level" / "9709" / "2024" / "w24" / "9709_w24_qp_12.pdf"
    ...
    document.save(pdf_path)
    document.close()
    sidecar = project_root / "data" / "processed_questions" / "cie" / "a_level" / "9709" / "2024" / "w24" / "12" / "qp" / "9709_w24_qp_12_q01.json"
    ...
    counts = backfill_processed_question_content(project_root / "data" / "processed_questions")
...
def test_backfill_processed_question_content_removes_ms_content(tmp_path: Path) -> None:
    ...
    sidecar = project_root / "data" / "processed_questions" / "cie" / "a_level" / "9709" / "2024" / "w24" / "12" / "ms" / "9709_w24_ms_12_q01.json"
    sidecar.parent.mkdir(parents=True)
    sidecar.write_text(
        json.dumps(
            {
...
def _load_9709_cutter_module():
    cutter_path = Path(__file__).resolve().parents[1] / "src" / "9709" / "cutter.py"
...
def _load_9231_cutter_module():
    cutter_path = Path(__file__).resolve().parents[1] / "src" / "9231" / "cutter.py"
