...
from cie_alevel_splitter.content_extraction import extract_content_from_fitz_clips
from cie_alevel_splitter.paths import find_project_root
...
def _local_source_pdf_path_from_url(source_pdf: str, project_root: Path) -> Path:
    ...
    current = project_root / "data" / "raw_papers" / "cie" / "a_level" / subject_code / str(year) / session / f"{stem}.pdf"
    if current.exists():
        return current
    legacy = project_root / "data" / "raw_papers" / "cie" / "a_level" / subject_code / session / f"{stem}.pdf"
