...
from cie_alevel_splitter.cie import load_assets
from cie_alevel_splitter.content_backfill import backfill_processed_question_content
from cie_alevel_splitter.pipeline import ingest_slices, split_asset_pdf
from cie_alevel_splitter.layout_splitter import SplitOptions, split_installed_paper_sets
from cie_alevel_splitter.database import connect
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_db.add_argument("--db", type=Path, default=Path("data/exam_bank.sqlite3"))

    split = subparsers.add_parser("split", help="切分 manifest 中的单个资源。 / Split one manifest asset.")
    split.add_argument("--manifest", type=Path, default=Path("configs/cie_a_level_manifest.json"))
    split.add_argument("--asset-index", type=int, default=0)
    split.add_argument("--raw-root", type=Path, default=Path("data/raw_papers"))
    split.add_argument("--processed-root", type=Path, default=Path("data/processed_questions"))

    split_all = subparsers.add_parser("split-downloaded", help="切分已下载且带 metadata 的 PDF。 / Split downloaded PDFs with metadata.")
    split_all.add_argument("--raw-root", type=Path, default=Path("data/raw_papers"))
    split_all.add_argument("--processed-root", type=Path, default=Path("data/processed_questions"))
    split_all.add_argument("--report", type=Path, default=Path("data/reports/cie_split_status.jsonl"))
    ...
    ingest.add_argument("--manifest", type=Path, default=Path("configs/cie_a_level_manifest.json"))
    ingest.add_argument("--asset-index", type=int, default=0)
    ingest.add_argument("--raw-root", type=Path, default=Path("data/raw_papers"))
    ingest.add_argument("--processed-root", type=Path, default=Path("data/processed_questions"))
    ingest.add_argument("--db", type=Path, default=Path("data/exam_bank.sqlite3"))
    ...
    backfill_content.add_argument("--processed-root", type=Path, default=Path("data/processed_questions"))
